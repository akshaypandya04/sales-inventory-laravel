...
@section('content')
    ...
    <main class="app-content">
        <div class="app-title">
            <div>
                <h1><i class="fa fa-edit"></i>Edit Purchase Entry</h1>
                <p>Sample forms</p>
            </div>
            <ul class="app-breadcrumb breadcrumb">
                <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i></li>
                <li class="breadcrumb-item">Purchase Entry</li>
                <li class="breadcrumb-item"><a href="#">Edit Purchase Entry</a></li>
            </ul>
        </div>
...
<form method="POST" action="{{ route('purchases.update', $purchase->id) }}">
    ...
    <div class="row">
        <div class="form-group col-md-4">
            ...
            <input name="invoice_no" class="form-control" id="invoice_no" type="text" value="{{ old('invoice_no', $purchase->invoice_no) }}" required>
        </div>
        ...
        <div class="form-group col-md-4">
            ...
            <select name="party_id" class="form-control party_id" id="party_id" required>
                ...
                @foreach($customers as $customer)
                    <option value="{{ $customer->id }}" {{ $purchase->party_id == $customer->id ? 'selected' : '' }}>
                        {{ $customer->name }}
                    </option>
                @endforeach
            </select> 
    ...
    <select name="category_id" class="form-control" id="category_id" required>
        ...
        @foreach($category as $value)
            <option value="{{ $value->id }}"
                {{ old('category_id', $purchase->category_id ?? '') == $value->id ? 'selected' : '' }}>
                {{ $value->category_name }}
            </option>
        @endforeach
    </select> 
...
<div class="form-group col-md-4">
    ...
    <select name="item_id" class="form-control" id="item_id" required>
        ...
        @foreach($items as $item)
            <option value="{{ $item->id }}"
                {{ old('item_id', $purchase->item_id ?? '') == $item->id ? 'selected' : '' }}>
                {{ $item->item_name }}
            </option>
        @endforeach
    </select>
        ...
        <div class="form-group col-md-4">
            ...
            <input name="purchase_price" class="form-control" id="amount" type="number" value="{{ old('purchase_price', $purchase->purchase_price) }}" placeholder="price">
        </div>

        <div class="form-group col-md-4">
            <label class="control-label">Qty</label>
            <input name="qty" class="form-control" id="qty" type="number" value="{{ old('qty', $purchase->qty) }}" placeholder="Enter Product Qty">
        </div>

        <div class="form-group col-md-4">
            <label class="control-label">Total</label>
            <input name="total" class="form-control" type="number" value="{{ old('total', $purchase->total) }}" placeholder="Enter Total" required>
            @error('total')
    ...
    <div class="row">
        ...
        <div class="form-group col-md-6">
            ...
            <textarea class="form-control" name="remarks" rows="10">{{ old('remarks', $purchase->remarks) }}</textarea>
        </div>
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                <input name="final_amount" class="form-control" id="final_amount" type="number" value="{{ old('final_amount', $purchase->final_amount) }}" required>
            </div>

            <div class="form-group">
                <label class="control-label">Paid Amount</label>
                <input name="paid_amount" class="form-control" id="paid_amount" type="number" value="{{ old('paid_amount', $purchase->paid_amount) }}" required>
            </div>

            <div class="form-group">
                <label class="control-label">Remain Amount</label>
                <input name="remain_amount" class="form-control" id="remain_amount" type="number" value="{{ old('remain_amount', $purchase->remain_amount) }}" required>
            </div>
        </div>
    </div>
...
<script>
    $(document).ready(function () {
        $('#category_id').on('change', function () {
            ...
            if (categoryId) {
                $.ajax({
                    url: '{{url("/get-by-category/") }}/' + categoryId,
                    type: 'GET',
